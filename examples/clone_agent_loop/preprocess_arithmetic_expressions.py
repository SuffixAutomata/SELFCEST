...
DEFAULT_SYSTEM_PROMPT = (
    ...
    "You are the root agent. Solve the arithmetic expression. "
    "Put the final answer in the format `Answer: <value>`."
)
...
def make_map_fn(split: str, data_source: str, system_prompt: str):
    def process_fn(example, idx: int):
        ...
        return {
            ...
            "agent_name": "tool_agent" if USE_CLONES else "",
            "prompt": [
...
if __name__ == "__main__":
    ...
    train_dataset.to_parquet(os.path.join(local_save_dir, "train.parquet" if USE_CLONES else "train_no_clones.parquet"))
    test_dataset.to_parquet(os.path.join(local_save_dir, "test.parquet" if USE_CLONES else "test_no_clones.parquet"))
